...
bool is_elf(Elf64_Ehdr eh )
{
	return eh.e_ident[EI_MAG0] == ELFMAG0 && eh.e_ident[EI_MAG1] == ELFMAG1 &&
			eh.e_ident[EI_MAG2] == ELFMAG2 && eh.e_ident[ELFMAG2] &&
...
void print_section_headers(int32_t fd, Elf64_Ehdr eh, Elf64_Shdr sh_table[])
{
	...
	for(i=0; i<eh.e_shnum; i++) {
		if(!strncmp((sh_str + sh_table[i].sh_name), ".rodata", 7)) { 
			ro = read_section(fd, sh_table[i]);  
			ro = realloc(ro, sh_table[i].sh_size +1); 
			ro[sh_table[i].sh_size] = 0; 
			char* p = ro; 

			while (p < ro+sh_table[i].sh_size) {  
				if ((modify = strstr(p,find)) != NULL) { 
					memset(modify, 0, strlen(find)); 
					memcpy(modify, "I modified it", strlen("I modified it")); 
					assert(lseek(fd, (off_t)sh_table[i].sh_offset, SEEK_SET) == (off_t)(sh_table[i].sh_offset)); 
					assert(write(fd, ro, sh_table[i].sh_size) == sh_table[i].sh_size); 
				}
				p += strlen(p) + 1; 
